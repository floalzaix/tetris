...
public class IA implements PropertyChangeListener {
    ...
    private final UnaryOperator<Float> alphaDecay;
    private final UnaryOperator<Float> epsilonDecay;
    private final UnaryOperator<Float> gammaDecay;
    ...
    public IA(float alphaInit, float epsilonInit, float gammaInit, UnaryOperator<Float> alphaDecay,
            UnaryOperator<Float> epsilonDecay, UnaryOperator<Float> gammaDecay, int largeurPuits, int profondeurPuits, int modeUsine) {
    ...
    private float getQMax(Etat etat) {
        return this.model.output(etat.get()).max(0).getFloat(0);
    }

    private INDArray getQValues(Etat etat) {
        return this.model.output(etat.get());
    }
        ...
        if (this.pose) {
            ...
            for (Coordonnees coordElement : coordsPiece) {
                ...
                for (int[] delta : new int[][] {{0, 1}, {1, 0}, {0, -1}, {-1, 0}}) {
                    ...
                    if (x < 0 || puits.getLargueur() <= x || y < 0 || puits.getProfondeur() <= y) { // Si le voisin est un mur
                        recompense+= 1;
                    } else {
    ...
    private Action getAction(Etat etat, Piece piece) {
        // EXPLORATION
        if (this.random.nextFloat() >= 1 - this.epsilon) {
            int indiceRandomAction = this.random.nextInt(Action.getNbActions());
            return Action.getAction(indiceRandomAction, piece);
        }

        // EXPLOITATION
        return Action.getAction((Nd4j.argMax(this.model.output(etat.get()), 0).getInt(0)), piece);
    }
    ...
    private void updateQValues(Etat etat, Action action, Etat prochainEtat, int recompense) {
        ...
        final float prochainEtatQMax = this.getQMax(prochainEtat);

        // Récupération des Q values de l'état actuel pour construire la target
        INDArray qValuesActuelles = this.getQValues(etat);
        float targetQ = recompense + this.gamma * prochainEtatQMax;
        qValuesActuelles.putScalar(0, indiceAction, targetQ);

        // Entrainement du modèle en le fesant tendre vers r + max Q (s', a')
        this.model.fit(etat.get(), qValuesActuelles);
    }

    private void updateHyperParametres() {
        this.alpha = this.alphaDecay.apply(alpha);
        this.gamma = this.gammaDecay.apply(gamma);
        this.epsilon = this.epsilonDecay.apply(epsilon);
    }
    ...
    public void train(int nbEpisodes) {
        ...
        for (int episode = 1; episode <= nbEpisodes; episode++) {
            ...
            while (!this.defaite) {
                ...
                this.updateHyperParametres();
            }

            //
            // Stats
            //
            int score = puits.getScore();
            scoreMoyen = (scoreMoyen * episode + score) / (episode + 1);
            scoreMax = Math.max(scoreMax, score);

            LOGGER.log(Level.INFO, "Episode {0} / {1} + stats : SCORE MOYEN {2} - SCORE MAX {3} - ALPHA {4} - GAMMA {5} - EPSILON {6}",
                    new float[] { episode, nbEpisodes, scoreMoyen, scoreMax, this.alpha, this.gamma, this.epsilon });
        }
